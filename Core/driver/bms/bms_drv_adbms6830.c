...
int adbms6830_read_all_cell_voltages(uint8_t num_slaves, uint8_t cells_per_slave, float cell_voltages[num_slaves][cells_per_slave])
{
    ...
    for (uint8_t slave_idx = 0U; slave_idx < num_slaves; slave_idx++)
    {
        ...
        {
            cell_voltages[slave_idx][cell_idx] = adbms6830_adc_to_volts(raw_cell_voltages[slave_idx][cell_idx]);
        }
    }
...
void adbms6830_read_half_cell_temps(uint8_t num_slaves, uint8_t therms_per_slave, float cell_temps[num_slaves][therms_per_slave], uint8_t mux_state)
{
    //loop through all gpios on both sides of the mux
    for(int mux_state = 0; mux_state < 1; mux_state++)
    {
        //start cell voltage ADCs
        int result = adbms6830_start_gpio_adc();

        //wait for conversion time
        osDelay(ADBMS_ADCV_CONV_TIME_MS);

        uint16_t raw_cell_temps[num_slaves][therms_per_slave];
        //read raw adc values from registers
        result = adbms6830_read_gpio_voltages_raw(raw_cell_temps, mux_state);
        ...
        for (int slave = 0; slave < ADBMS_NUM_SLAVES; slave++)
        {
            ...
            {   
                float thermistor_voltage = adbms6830_adc_to_volts(raw_cell_temps[slave][thermistor]);
                cell_temps[slave][thermistor] = calculate_thermistor_temperature(thermistor_voltage);
            }
