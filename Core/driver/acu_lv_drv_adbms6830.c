...
{
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef hal_status = HAL_SPI_Transmit(ADBMS6830_SPI_HANDLE,
                                                    (uint8_t *)cmd_frame, 4U, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_SET);
    return hal_status;
}
 ...
 * @return HAL_OK on success, HAL error code on failure.
 */
static HAL_StatusTypeDef adbms_spi_read(const uint8_t cmd_frame[4],
                                        uint8_t *rx_data, uint16_t rx_len)
{
    ...
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef hal_status = HAL_SPI_TransmitReceive(ADBMS6830_SPI_HANDLE,
                                                           tx_buf, rx_buf, total_len, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_SET);
 ...
 * @return HAL_OK on success, HAL error code on failure.
 */
static HAL_StatusTypeDef adbms_spi_write(const uint8_t cmd_frame[4],
                                         const uint8_t *data, uint16_t data_len)
{
    ...
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_RESET);
    HAL_StatusTypeDef hal_status = HAL_SPI_Transmit(ADBMS6830_SPI_HANDLE,
                                                    tx_buf, 4U + data_len, HAL_MAX_DELAY);
    HAL_GPIO_WritePin(ADBMS6830_CS_GPIO_Port, ADBMS6830_CS_Pin, GPIO_PIN_SET);
    return hal_status;
}
 ...
 */
static int adbms6830_read_register_group(uint16_t cmd,
                                         uint8_t rx_data[ADBMS_NUM_SLAVES][ADBMS_REG_GROUP_SIZE])
{
    /* Command frame */
    uint8_t cmd_buf[4];
    cmd_buf[0] = (uint8_t)(cmd >> 8);
...
int adbms6830_read_cell_voltages_raw(uint16_t raw_adc[ADBMS_NUM_SLAVES][ADBMS_CELLS_PER_IC])
{
    ...
    /* Lock SPI bus for entire multi-register read operation */
    spi_lock();
