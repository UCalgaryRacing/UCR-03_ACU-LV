...
mcu_analog_runtime_config_t mcu_analog_runtime_config[MCU_MAX_ADC_COUNT];

static inline bool is_valid_channel_id(mcu_analog_channel_id_t channel_id)
{
    return (channel_id < MCU_MAX_ADC_COUNT);
}
...
const mcu_analog_config_t* mcu_svc_get_channel_config(mcu_analog_channel_id_t channel_id)
{
    ...
    return &mcu_adc_config[channel_id];
}
...
// NOT USED: no current settings (active/valid max, min)
